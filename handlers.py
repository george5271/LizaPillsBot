...
from config import ADMIN_CHAT_ID, DEFAULT_PILL_INTERVAL, DEFAULT_PILL_SCHEDULE, LIZA_CHAT_ID, TIMEZONE
from scheduler import Delivery, choose, get_keyboard
...
def create_router(bot, storage: DataStorage, delivery: Delivery) -> Router:
    router = Router()
    ...
    async def cmd_send_message_image(message: Message):
        ...
        if not text:
            ...
            return
        try:
            await delivery.send_both(photo=choose(IMAGES['day']), text=text)
    ...
    async def process_admin_message(message: Message, state: FSMContext):
        ...
        try:
            if message.photo:
                ...
                await delivery.send_both(text=message.caption, photo=photo_id)
            elif message.text:
                await delivery.send_both(text=message.text)
            else:
    ...
    async def btn_taken(message: Message):
        ...
        if not storage.mark_day('taken'):
            return
        await delivery.send_both(photo=choose(IMAGES['day']), text=random.choice(TEXTS['taken']))
        if storage.data.get('streak', 0) > 1:
            await delivery.send_both(text=f"🔥 Стрик: {storage.data['streak']} дней!")
    ...
    async def btn_missed(message: Message):
        ...
        if not storage.mark_day('missed'):
            return
        await delivery.send_both(photo=choose(IMAGES['day']), text=random.choice(TEXTS['missed']))
